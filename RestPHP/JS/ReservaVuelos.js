...
ReservaVuelos.prototype.actualizarVuelos = function() {
    ...
    var tablevuelogen=$("#tablaVuelos").find("tbody");
      tablevuelogen.empty();
    $.ajax({
        "url": "PHP/VueloGenerico.php?param="+JSON.stringify(buscarVueloGen),
        "type": "POST",
        "dataType": "json",
        "success": function(vuelos) {
            ...
            if (vuelos.length===0) {
                ...
                $.each(vuelos, function(i, vuelo) {
                    ...
                    var tr = $('<tr/>')
                        .append(
                          $("<td>").text(vuelo.nVuelo),
                        $("<td>").text(vuelo.horaSalida),
                        $("<td>").text(vuelo.horaLlegada),
                        $("<td>").text("Origen"),
                        $("<td>").text("Destino"),
                        $("<td>").text("Aerolinea"),
                        $("<td>").text(vuelo.precio)
                        )
                        .appendTo(tablevuelogen);
                });
            }
        }
    });
        ...
        success: function(aerolineas) {
          if(aerolineas.length===0){
            ...
            $.each(aerolineas, function(i, aerolineas) {
              var item = $('<option value='+aerolineas.aerolinea_id+'>'+aerolineas.nombre+'</option>')
                  .appendTo(select);
            });
          }
        },
